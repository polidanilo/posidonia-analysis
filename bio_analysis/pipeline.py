"""
Pipeline - Coordina moduli e fasi operative
"""
...
class AnalysisPipeline:
    """
    Pipeline per l'analisi di Posidonia oceanica.
    Supporta workflow per singolo file e unione multipla (tiled).
    """
    ...
    def run_single_file(
        ...
        """
        Esegue analisi su file singolo.
        """
        logger.info("\n" + "="*70)
        ...
        try:
            ...
            xlsx_path = self.reporter.export_excel(
                metrics, f"{output_prefix}.xlsx"
            )
            
            png_path = self.reporter.export_png(
                metrics, warnings, lum_grid, alt_grid, mask_pos, celle_val, f"{output_prefix}.png"
            )
    ...
    def run_tiled_ply(
        ...
        """
        Esegue analisi su un insieme di tile da unire.
        """
        logger.info("\n" + "="*70)
        logger.info("INIZIO ANALISI MULTIPLA (MERGE AUTOMATICO)")
        logger.info("="*70)
        
        try:
            logger.info(f"\n[1/6] Loading & Merging PLY tiles: {ply_folder}")
            ...
            xlsx_path = self.reporter.export_excel(
                metrics, seg_info, self.calibrator.get_calibration_report(), f"{output_prefix}.xlsx"
            )
            ...
            logger.info("\n" + "="*70)
            logger.info("ANALISI MULTIPLA COMPLETATA CON SUCCESSO")
            logger.info("="*70)

...
with open(file_images, "r") as f:
    for riga in f:
        ...
        if len(parti) >= 10 and (parti[-1].lower().endswith(".jpg") or parti[-1].lower().endswith(".png")):
            # COLMAP salva le coordinate. Le estraiamo e le scaliamo in metri reali
...
# 3. Salva in CSV
np.savetxt("traiettoria.csv", percorso, delimiter=",", header="X,Y,Z", comments="")
print(f"Fatto! Salvate {len(percorso)} posizioni in 'traiettoria.csv'")